...
void fftr2(MFFTELEM **YY, MFFTELEM **XX, int64_t N,
           int32_t e1, int64_t bp, int64_t stride, int32_t flags) {
...
void fftr5(MFFTELEM **YY, MFFTELEM **XX, int64_t N,
  ...
  for (int32_t t = 0; t < e1; t++) {
    ...
    for (int64_t j = 0; j < l; j++) {
      const std::complex<double> w2 = w * w;
      const std::complex<double> w3 = w2 * w;
      const std::complex<double> w4 = w3 * w;
      for (int64_t k = 0; k < m; k++) {
        c0 = X[bp + stride * (k + j * m)];
        c1 = X[bp + stride * (k + j * m + l * m)];
        c2 = X[bp + stride * (k + j * m + 2 * l * m)];
...
void fftr7(MFFTELEM **YY, MFFTELEM **XX, int64_t N,
  ...
  for (int32_t t = 0; t < e1; t++) {
    ...
    for (int64_t j = 0; j < l; j++) {
      ...
      const std::complex<double> w3 = w2 * w;
      const std::complex<double> w4 = w3 * w;
      const std::complex<double> w5 = w4 * w;
      const std::complex<double> w6 = w5 * w;
      for (int64_t k = 0; k < m; k++) {
        c0 = X[bp + stride * (k + j * m)];
        c1 = X[bp + stride * (k + j * m + l * m)];
        c2 = X[bp + stride * (k + j * m + 2 * l * m)];
...
void fftr8(MFFTELEM **YY, MFFTELEM **XX, int64_t N,
  ...
  for (int32_t t = 0; t < e1; t++) {
    ...
    for (int64_t j = 0; j < l; j++) {
      ...
      const std::complex<double> w5 = w4 * w;
      const std::complex<double> w6 = w5 * w;
      const std::complex<double> w7 = w6 * w;
      for (int64_t k = 0; k < m; k++) {
        c0 = X[bp + stride * (k + j * m)];
        c1 = X[bp + stride * (k + j * m + l * m)];
        c2 = X[bp + stride * (k + j * m + 2 * l * m)];
...
void fftr9(MFFTELEM **YY, MFFTELEM **XX, int64_t N,
  ...
  for (int32_t t = 0; t < e1; t++) {
    ...
    for (int64_t j = 0; j < l; j++) {
      ...
      const std::complex<double> w6 = w5 * w;
      const std::complex<double> w7 = w6 * w;
      const std::complex<double> w8 = w7 * w;
      for (int64_t k = 0; k < m; k++) {
        c0 = X[bp + stride * (k + j * m)];
        c1 = X[bp + stride * (k + j * m + l * m)];
        c2 = X[bp + stride * (k + j * m + 2 * l * m)];
