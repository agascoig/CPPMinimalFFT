...
#define CCDPTR(x) \
  reinterpret_cast<const double *__restrict__>(__builtin_assume_aligned(x, 16))
#define CDPTR(x) \
  reinterpret_cast<double *__restrict__>(__builtin_assume_aligned(x, 16))
...
void fftr2(MFFTELEM **YY, MFFTELEM **XX, const int64_t N, const int32_t e1,
  ...
  auto pmim_mask = hn::Load(d, conj_values);
  const auto conj_mask = pmim_mask;
  ...
  for (int32_t t = 0; t < e1; t++) {
    ...
    for (int64_t j = 0; j < l; j++) {
      ...
      for (int64_t k = 0; k < m; k++) {
        ...
        auto c1 = hn::Load(d, CCDPTR(&X[bp + stride * (k + j * m + l * m)]));
        auto c2 =
            hn::Load(d, CCDPTR(&X[bp + stride * (k + j * m + 2 * l * m)]));
  ...
  for (int32_t t = 0; t < e1; t++) {
    ...
    for (int64_t j = 0; j < l; j++) {
      ...
      for (int64_t k = 0; k < m; k++) {
        ...
        auto c2 =
            hn::Load(d, CCDPTR(&X[bp + stride * (k + j * m + 2 * m * l)]));
        auto c3 =
            hn::Load(d, CCDPTR(&X[bp + stride * (k + j * m + 3 * m * l)]));
    ...
    for (int64_t j = 0; j < l; j++) {
      ...
      for (int64_t k = 0; k < m; k++) {
        ...
        auto c2 =
            hn::Load(d, CCDPTR(&X[bp + stride * (k + j * m + 2 * l * m)]));
        auto c3 =
            hn::Load(d, CCDPTR(&X[bp + stride * (k + j * m + 3 * l * m)]));
        auto c4 =
            hn::Load(d, CCDPTR(&X[bp + stride * (k + j * m + 4 * l * m)]));
    ...
    for (int64_t j = 0; j < l; j++) {
      ...
      for (int64_t k = 0; k < m; k++) {
        ...
        auto c4 =
            hn::Load(d, CCDPTR(&X[bp + stride * (k + j * m + 4 * l * m)]));
        auto c5 =
            hn::Load(d, CCDPTR(&X[bp + stride * (k + j * m + 5 * l * m)]));
        auto c6 =
            hn::Load(d, CCDPTR(&X[bp + stride * (k + j * m + 6 * l * m)]));
    ...
    for (int64_t j = 0; j < l; j++) {
      ...
      for (int64_t k = 0; k < m; k++) {
        ...
        auto c6 =
            hn::Load(d, CCDPTR(&X[bp + stride * (k + j * m + 6 * l * m)]));
        auto c7 =
            hn::Load(d, CCDPTR(&X[bp + stride * (k + j * m + 7 * l * m)]));
    ...
    for (int64_t j = 0; j < l; j++) {
      ...
      for (int64_t k = 0; k < m; k++) {
        ...
        auto c7 =
            hn::Load(d, CCDPTR(&X[bp + stride * (k + j * m + 7 * l * m)]));
        auto c8 =
            hn::Load(d, CCDPTR(&X[bp + stride * (k + j * m + 8 * l * m)]));
