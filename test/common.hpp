...
static void print_fns(char* buf, fft_func_t* fns) {
  ...
  buf[strlen(buf) - 1] = 0;  // remove last space, or open paren
...
fftwf_plan create_fftw_multid_plan(int dims, int64_t* Ns,
  ...
  if (region_start == 0 && region_end == dims - 1) {
    ...
    auto P = fftwf_plan_dft(dims, Ns_rev, (fftwf_complex*)in,
                            (fftwf_complex*)out, dir, FFTW_ESTIMATE);
    minassert(P != nullptr, "Failed to generate FFTW plan.");
    delete[] Ns_rev;  // safe
    return P;
  }
 // construct guru level plan
  std::vector<int> stride(dims);
  stride[0] = 1;
  for (int i = 1; i < dims; i++) {
    stride[i] = stride[i - 1] * Ns[i - 1];
  }
  std::vector<fftw_iodim> transform_dims;
  for (int i = region_start; i <= region_end; i++) {
    fftw_iodim d;
    d.n = Ns[i];
    d.is = stride[i];
    d.os = stride[i];
    transform_dims.push_back(d);
  }
  ...
  for (int i = region_end + 1; i < dims; i++) {
    fftw_iodim d;
    d.n = Ns[i];
    d.is = stride[i];
    d.os = stride[i];
  ...
  auto P = fftwf_plan_guru_dft(
      transform_dims.size(), transform_dims.data(), howmany_dims.size(),
      howmany_dims.data(), reinterpret_cast<fftwf_complex*>(in),
      reinterpret_cast<fftwf_complex*>(out), dir, FFTW_ESTIMATE);
  minassert(P != nullptr, "Failed to generate partial multidim FFTW plan.");
  return P;
}
...
fftw_plan create_fftw_multid_plan(int dims, int64_t* Ns,
  ...
  if (region_start == 0 && region_end == dims - 1) {
    int* Ns_rev = new int[dims];  // reverse for row-major FFTW
    for (int i = 0; i < dims; ++i) Ns_rev[i] = Ns[dims - i - 1];
    auto P = fftw_plan_dft(dims, Ns_rev, (fftw_complex*)in, (fftw_complex*)out,
                           dir, FFTW_ESTIMATE);
    minassert(P != nullptr, "Failed to generate FFTW plan.");
    delete[] Ns_rev;  // safe
    return P;
  ...
  auto P = fftw_plan_guru_dft(
      transform_dims.size(), transform_dims.data(), howmany_dims.size(),
      howmany_dims.data(), reinterpret_cast<fftw_complex*>(in),
      reinterpret_cast<fftw_complex*>(out), dir, FFTW_ESTIMATE);
  minassert(P != nullptr, "Failed to generate partial multidim FFTW plan.");
  return P;
}
...
void execute_minimal_test_planned(MinimalPlan* P, MinAlignedVector& Y,
                                  MinAlignedVector& X) {
...
void execute_minimal_test_unplanned(MinAlignedVector& Y, MinAlignedVector& X,
                                    bool pfa, int64_t N, int64_t* Ns, int nf,
                                    fft_func_t* fns, int64_t* base, int32_t* es,
                                    const int64_t* QPs, const MAP_CACHE_T* nm,
                                    const MAP_CACHE_T* km, int inverse) {
  ...
  if (!pfa) {
    ...
    region_data rd = create_region_data(Ns, nf, fns, base, es, QPs, nm, km);
    prime_factor(YY, XX, rd, 0, 1, inverse, nf);
  } else {
...
void test_fft_kernel_untimed(int64_t repeat_count, MinAlignedVector& Y_ref,
                             MinAlignedVector& Y, MinAlignedVector& X_ref,
                             MinAlignedVector& X, MinAlignedVector& copy_X,
                             auto& P_ref, MinimalPlan* P, int64_t N, int32_t bm,
                             double* t_ref_s, double* t_s, char nf, int64_t* Ns,
                             fft_func_t* fns, int64_t* base, int32_t* es,
                             bool pfa, int32_t inverse, double* std_dev,
                             const int64_t* QPs, const MAP_CACHE_T* nm,
...
void test_fft_kernel_timed(int64_t repeat_count, MinAlignedVector& Y_ref,
                           MinAlignedVector& Y, MinAlignedVector& X_ref,
                           MinAlignedVector& X, MinAlignedVector& copy_X,
                           auto& P_ref, MinimalPlan* P, int64_t N, int32_t bm,
                           double* t_ref_s, double* t_s, char nf, int64_t* Ns,
  ...
  *t_s = 0.0;
  ...
  // parity with FFTW timing: pre-execute once
  if (P)
    execute_minimal_test_planned(P, Y, X);
  else
    execute_minimal_test_unplanned(Y, X, pfa, N, Ns, nf, fns, base, es, QPs, nm,
...
void test_fft_kernel(int64_t repeat_count, MinAlignedVector& Y_ref,
                     MinAlignedVector& Y, MinAlignedVector& X_ref,
                     MinAlignedVector& X, MinAlignedVector& copy_X, auto& P_ref,
                     MinimalPlan* P, int64_t N, int32_t bm, double* t_ref_s,
                     double* t_s, int32_t nf, int64_t* Ns, fft_func_t* fns,
                     int64_t* base, int32_t* es, bool pfa, int32_t inverse,
                     double* std_dev, const int64_t* QPs, const MAP_CACHE_T* nm,
                     const MAP_CACHE_T* km) {
  // dispatch time or untimed
  if (bm) {
    test_fft_kernel_timed(repeat_count, Y_ref, Y, X_ref, X, copy_X, P_ref, P, N,
                          bm, t_ref_s, t_s, nf, Ns, fns, base, es, pfa, inverse,
                          std_dev, QPs, nm, km);
  } else {
    test_fft_kernel_untimed(repeat_count, Y_ref, Y, X_ref, X, copy_X, P_ref, P,
                            N, bm, t_ref_s, t_s, nf, Ns, fns, base, es, pfa,
                            inverse, std_dev, QPs, nm, km);
  }
}

void print_test(const char* preamble, const char* name, int64_t N, int nf,
                  int64_t* Ns, fft_func_t* fns) {
 char fn_str[256];
  fn_str[0] = '\0';
  ...
  for (int i = 0; i < nf; i++) {
   ...
   printf("%s %s %s] %s\n", preamble, name, factors_str, fn_str);  
}

void print_result(const char* preamble, const char* name, int64_t N, int nf,
                  int64_t* Ns, int bm, double t_ref_s, double t_s,
                  fft_func_t* fns, double std_dev) {
  char timing_str[256];
  timing_str[0] = '\0';
  if (bm) {
  ...
  if (bm) {
    ...
    printf("%s %s %s %s] %s\n", preamble, name, timing_str, factors_str,
           fn_str);
  }
  fflush(stdout);
}
...
void test_fft(random_normal& RNG, const char* name, int bm, int inverse,
              int64_t N, int* pc, int* fc, int nf, int dims, bool pfa,
              MinimalPlan* P, int64_t* Ns, fft_func_t* fns, int64_t* base,
              int32_t* es) {
  ...
  if (dims > 1) {
    P_ref =
        create_fftw_multid_plan(dims, Ns, X_ref.data(), Y_ref.data(), P->get_region_start(), P->get_region_end(), inverse);
  } else
  ...
  if (nf > 1) {
    if (P) {
      ...
      int64_t params[MAX_PFA_PARAMS];
      QPs(params, nf, Ns);
      MAP_CACHE_T* nm =
          (N <= MinPlanConfig.max_map_cache ? generate_nmap(nf, N, Ns, params)
                                            : nullptr);
      MAP_CACHE_T* km =
...
std::string gen_key(bool pfa, void* fns, int64_t* N_vals, int32_t* es,
                    int32_t inverse) {
  std::string key;
  ...
  key.append(reinterpret_cast<const char*>(&inverse), sizeof(int32_t));
  return key;
}
...
void driver(random_normal& RNG, hashmap_t& d, int* radix, int radix_count,
            int nf, int bm, int* pc, int* fc, int32_t inverse, bool pfa,
            const int64_t* N_vals, int N_val_count, const char* name,
            int64_t direct_sz) {
  ...
  for (int t = 0; t < N_val_count; t++) {
    ...
    for (int f = 0; f < nf; f++) {
      ...
      std::string key = gen_key(pfa, fns, Ns, es, inverse);
      if (d.find(key) != d.end() || i != nf) continue;
      d[key] = 1;
      test_fft(RNG, name, bm, inverse, prod(Ns, nf), pc, fc, nf, 1, pfa, 0, Ns,
               fns, bs, es);
    }
  }
}
...
void enable_fp_exceptions() {
  ...
  feenableexcept(FE_INVALID | FE_DIVBYZERO |
                 FE_OVERFLOW);  // optional, may be stub
}
