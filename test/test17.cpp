...
MFFTELEM *test_fft_kernel(int repeat_count, MFFTELEM *Y_ref, MFFTELEM *Y,
  ...
  while (repeat_count-- > 0) {
    if (P != NULL) {
      ...
      if (num_factors == 1) {
        ...
        ((parent_fn_t)parent_fn)(&Y, &X, es, Ns, fns, 0, 1, inverse);
...
void print_result(const char *preamble, const char *name, int64_t N,
  ...
  char timing[256];
  if (bm) {
    snprintf(timing, sizeof(timing), "time = %2.2e factor_ref = %2.2e", t_s,
             t_s / t_ref_s);
  } else {
    snprintf(timing, sizeof(timing), "untimed");
  }
...
void test_fft(random_normal &RNG, const char *name, int bm, int inverse,
  ...
  MFFTELEM *Y_result =
      test_fft_kernel(test_repeat, Y_ref.data(), Y.data(), X_ref.data(),
                      ...
                      num_factors, Ns, fns, es, parent_fn, inverse);
  
  // reporting
  if (approx_cmp_v(Y_ref.data(), Y_result, N)) {
...
static const int64_t factor_2[][2] = {
    {4, 25}, {25, 4}, {4, 49}, {8, 9}, {256, 25}, {16, 5}, {8, 7},
    {11, 8}, {49, 3}, {9, 8}, {25, 256}, {1, 256}};
...
void driver(random_normal &RNG, hashmap_t &d, int *radix, int radix_count,
  ...
  for (int t = 0; t < N_val_count; t++) {
    ...
    for (int f = 0; f < num_factors; f++) {
      ...
      if (d.find(key) != d.end() || i != num_factors)
        continue;
      d[key] = 1;
...
#define RUN_DRIVER(radix_arr, num_factors, bm, inverse, parent_fn, N_vals,     \
  ...
  for (int i = 0; i < sizeof(planner_n) / sizeof(planner_n[0]); ++i) {
    ...
    test_fft(RNG, "planner", 1, 0, N, pc, fc, 0, NULL, &P, &N, P.func[region], NULL);

    MinimalPlan P_inv(N_p, 1, region, region, P_INVERSE);

    test_fft(RNG, "planner inverse", 1, 1, N, pc, fc, 0, NULL, &P_inv, &N, P_inv.func[region],
             NULL);
  }
