...
void MinimalPlan::plan_1d(int64_t n, int32_t rd) {
  ...
  if (n <= DIRECT_SZ) {
    ...
    for (int32_t i = factor_count - 1; i >= 0; i--) {
...
void MinimalPlan::execute_plan(MFFTELEM *Y, MFFTELEM *X, int64_t r, int64_t bp,
  ...
  minassert(Y!=X || (flags & P_INPLACE), "Input and output buffers must not be the same unless in-place plan.");
  ...
  if (*YY != orig_Y)
    memcpy(orig_Y, *YY, total_size * sizeof(MFFTELEM)); // swapped, so copy to original output
  else if (flags & P_INPLACE)
    memcpy(X, *YY, total_size * sizeof(MFFTELEM)); // in-place, so copy back to original input if not already
  if (copy_X)
    free(copy_X);
}
