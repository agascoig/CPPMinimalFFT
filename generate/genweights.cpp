...
void print_value(const int N, int wrap, int &i, const mpfr_t &c, const mpfr_t &s) {
  std::cout << c << "," << s;
  if (i != (N - 1)) {
    std::cout << ",";
    if ((++i%wrap) == 0) {
...
void print_value(const int N, int wrap, int &i, const double &c, const double &s) {
  std::cout << c << "," << s;
  if (i != (N - 1)) {
    std::cout << ",";
    if ((++i%wrap) == 0) {
...
int main() {
  ...
  std::string preamble = "\nalignas (sizeof(double)*2) static const double COS_SIN_";
