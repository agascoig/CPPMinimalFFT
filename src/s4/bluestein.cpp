...
void trap_large_v(const MFFTELEM *__restrict__ x, int line) noexcept
{
  int64_t M = bs_buff.M;
  for (int i = 0; i < M; ++i)
  {
...
void bluestein_init(int64_t N, int64_t M, int32_t flags) noexcept {
  ...
  if (init || bs_buff.N != N) {
    ...
    for (int64_t n = 1; n < N; n++) {
      c_e = prescale * std::exp(std::complex<float>(0.0, arg * n * n));
      b_n[n] = (MFFTELEM)c_e;
...
void bluestein(MFFTELEM** YY, MFFTELEM** XX, const int64_t N, const int32_t discard_e1,
  ...
  for (int64_t n = 0; n < N - 1; n += 2) {
    ...
    auto c = hn::ComplexConj(bv);
    auto xv = LoadComplexGroup(sp_4, &x[bp + stride * n], stride);
    auto a = hn::MulComplex(xv, c);
    hn::Store(a, sp_4, CFPTR(&a_n[n]));
    StoreComplexGroup(sp_4, c, &y[bp + stride * n], stride);
  }
  if (N & 1) {
    auto b = b_n[N - 1];
    auto c = std::conj(b);
  ...
  for (int64_t i = 0; i < N - 1; i += 2) {
    ...
    auto yv = LoadComplexGroup(sp_4, &y[bp + stride * i], stride);
    yv = hn::MulComplex(yv, BXv);
    yv = hn::Mul(yv, scale_v);
    StoreComplexGroup(sp_4, yv, &y[bp + stride * i], stride);
  }
  if (N & 1) {
    MFFTELEM BX = B_X[N - 1];
