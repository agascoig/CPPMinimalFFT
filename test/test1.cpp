...
bool test_column(bool inverse, MinimalPlan& P, int64_t N, int column) {
  ...
  for (int i = 0; i < N; ++i) {
    if (inverse)
      Y_ref[i] = std::exp(std::complex<MFFTELEMRI>(0,2 * M_PI * column * i / N));
    else
      Y_ref[i] = std::exp(std::complex<MFFTELEMRI>(0,-2 * M_PI * i * column / N));
  }
...
int driver(int &pass, int &fail, int _direct_sz, int _small_sz) {
  // planner control
...
int main(int argc, char **argv) {
  ...
  driver(pass, fail, 0, 0);
  driver(pass, fail, DEFAULT_DIRECT_SZ, 0);
  driver(pass, fail, 0, DEFAULT_SMALL_SZ);
